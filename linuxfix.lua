...
function check_bind(key_data)
	...
	return check_bind_table(key_data, keybinds) or check_bind_table(key_data, keybinds[chat_open and 'ChatOnly' or 'NoChat'])
end

function check_bind_table(key_data, t)
	if mods:length() == 1 then
		t = t and t[mods:it()()]
	end
	t = t and t[key_data.name]
...
windower.register_event('incoming text', function(original, modified, color, color_m, blocked)
	...
	elseif original == '===== Listing Currently Bound Keys =====' then
		...
		keybinds = T{}
		ListBinds = true --The listbinds text doesn't have any unique identifying features so I added this to only capture text while listbinds is running.
	end
	return true
end)

local chat_states = T{['(NoChat)']='NoChat', ['(ChatOnly)']='ChatOnly'}
function add_bind(original)
	local parts = {original:match('^([^%-:]*)%-?([^: ]*): (.*)$')}
	local command = table.last(parts)
	table.remove(parts)
	local chat_state, mod, key
